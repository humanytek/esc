...
class res_currency_fit(osv.osv):
    ...
    _columns = {'name': fields.char('Nombre', size=128, required=True),
     'code': fields.char('Codigo SAT', size=10, required=True)}
    ...
    def create(self, cr, uid, vals, context):
        if 'allow_management' not in context or not context['allow_management']:
            raise osv.except_osv('Operacion no definida', 'Las monedas no pueden ser creadas manualmente.')
        return super(res_currency_fit, self).create(cr, uid, vals, context)

    def write(self, cr, uid, ids, vals, context):
        if 'allow_management' not in context or not context['allow_management']:
            raise osv.except_osv('Operacion no definida', 'Las monedas no pueden ser modificadas manualmente.')
        return super(res_currency_fit, self).write(cr, uid, ids, vals, context)

    def unlink(self, cr, uid, ids, context):
        if 'allow_management' not in context or not context['allow_management']:
            raise osv.except_osv('Operacion no definida', 'Las monedas no pueden ser eliminadas manualmente.')
...
class res_currency_sat(osv.osv):
    ...
    _columns = {'sat_currency_id': fields.many2one('eaccount.currency', 'Codigo del SAT', required=True)}
