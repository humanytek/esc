...
class sat_account_code(osv.osv):
    ...
    _description = 'Codigo agrupador de SAT para las cuentas'
    _columns = {'key': fields.char('Codigo agrupador', size=10, required=True),
    ...
    def create(self, cr, uid, vals, context):
        if 'allow_management' not in context or not context['allow_management']:
            raise osv.except_osv('Operacion no definida', 'Los codigos agrupadores no pueden ser creados manualmente.')
        return super(sat_account_code, self).create(cr, uid, vals, context)

    def write(self, cr, uid, ids, vals, context):
        if 'allow_management' not in context or not context['allow_management']:
            raise osv.except_osv('Operacion no definida', 'Los codigos agrupadores no son directamente modificables.')
        return super(sat_account_code, self).write(cr, uid, ids, vals, context)

    def unlink(self, cr, uid, ids, context):
        if 'allow_management' not in context or not context['allow_management']:
            raise osv.except_osv('Operacion no definida', 'Los codigos agrupadores no son directamente eliminables.')
