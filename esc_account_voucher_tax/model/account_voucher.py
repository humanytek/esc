...
class account_voucher(osv.Model):
    ...
    def onchange_amount(self, cr, uid, ids, amount, rate, partner_id, journal_id, currency_id, ttype, date, payment_rate_currency_id, company_id, context=None):
        res = super(account_voucher, self).onchange_amount(cr, uid, ids, amount, rate, partner_id, journal_id, 
            currency_id, ttype, date, payment_rate_currency_id, company_id, context=context)
        res_compute = self.onchange_compute_tax(cr, uid, ids, res, ttype, date, context=context)
        return res_compute
    ...
    def get_percent_pay_vs_invoice(self, cr, uid, amount_original, amount, context=None):
        return amount_original != 0 and float(amount) / float(amount_original) or 1.0
            ...
            for line in voucher.line_ids:
                ...
                for line_tax in line.tax_line_ids:
                    ...
                    move_lines_tax = self._preparate_move_line_tax(cr, uid,
                        ...
                        line_tax.analytic_account_id and line_tax.analytic_account_id.id or False,
                        line_tax.amount_base, factor, context=context)
                    for move_line_tax in move_lines_tax:
                        move_create = move_line_obj.create(cr ,uid, move_line_tax, context=context)
    ...
    def _preparate_move_line_tax(self, cr, uid, src_account_id, dest_account_id,
                            ...
                            tax_id, line_tax, acc_a, amount_base_tax, factor=0, context=None):
        ...
        amount_base, tax_secondary = self._get_base_amount_tax_secondary(cr, uid, line_tax, amount_base_tax * factor, reference_amount, context=context)
        debit_line_vals = {
            'name': line_tax.name,
            'quantity': 1,
            'partner_id': partner,
        ...
        if type in ('payment','purchase'):
            reference_amount < 0 and [credit_line_vals.pop('analytic_account_id'), 
                credit_line_vals.update({'amount_base': abs(amount_base), 'tax_id_secondary': tax_secondary})] or [debit_line_vals.pop('analytic_account_id'),
                debit_line_vals.update({'tax_id_secondary': tax_secondary, 'amount_base': abs(amount_base)})]
        else:
            reference_amount < 0 and [debit_line_vals.pop('analytic_account_id'), debit_line_vals.pop('tax_id_secondary')] or credit_line_vals.pop('analytic_account_id')
    ...
    def _get_base_amount_tax_secondary(self, cr, uid, line_tax,
        ...
        amount_base = 0
        tax_secondary = False
        if line_tax and line_tax.tax_category_id and line_tax.tax_category_id.name in ('IVA', 'IVA-EXENTO', 'IVA-RET', 'IVA-PART'):
            amount_base = line_tax.tax_category_id.value_tax and reference_amount/line_tax.tax_category_id.value_tax or amount_base_tax
    ...
    def onchange_compute_tax(self, cr, uid, ids, lines=None, ttype=False, date=False, context=None):
        invoice_obj = self.pool.get('account.invoice')
        currency_obj = self.pool.get('res.currency')
                        ...
                        for tax in invoice.tax_line:
                            if tax.tax_id.tax_voucher_ok:
                                ...
                                if company_currency == current_currency:
                                    ...
                                    credit_amount = currency_obj.compute(cr, uid, invoice.currency_id.id, current_currency, 
                                        float('%.*f' % (2,credit_amount)), round=False, context=context)
                                    amount_unround = currency_obj.compute(cr, uid, invoice.currency_id.id, current_currency, 
                                        float(amount_unround), round=False, context=context)
                                    base_amount_curr=currency_obj.compute(cr, uid, invoice.currency_id.id, current_currency, 
                                        float('%.*f' % (2,base_amount)), round=False, context=context)
                                    context['date'] = invoice.date_invoice
                                    credit_orig = currency_obj.compute(cr, uid, current_currency,company_currency, 
                                        float('%.*f' % (2,credit_amount)), round=False, context=context)
                                    context['date'] = date
                                    credit_diff = currency_obj.compute(cr, uid, current_currency,company_currency, 
                                        float('%.*f' % (2,credit_amount)), round=False, context=context)
                                    diff_amount_tax = currency_obj.compute(cr, uid, company_currency,current_currency, 
                                        float('%.*f' % (2,(credit_orig-credit_diff))), round=False, context=context)
                                ...
                                debit_amount = 0.0
                                move_line_id = False
                                line_ids = move_obj.browse(cr, uid, line['move_line_id'], context=context)
                                if line_ids:
                                    move_line_ids = line_ids.move_id and line_ids.move_id.line_id
                                    for move_lines in move_line_ids:
    ...
    def compute_tax(self, cr, uid, ids, context=None):
        ...
        for voucher_obj in self.browse(cr, uid, ids, context=context):
            company_currency = self._get_company_currency(cr, uid, voucher_obj.id, context)
            current_currency = self._get_current_currency(cr, uid, voucher_obj.id, context)
                    ...
                    for invoice in invoice_obj.browse(cr, uid, invoice_ids, context=context):
                        for tax in invoice.tax_line:
                            if tax.tax_id.tax_voucher_ok:
                                ...
                                _logger.warn('Valor de move_line_id %s', line['move_line_id']['id'])
                                
                                line_ids = move_obj.browse(cr, uid, line['move_line_id']['id'], context=context)
                                if line_ids:
                                    move_line_ids = line_ids.move_id and line_ids.move_id.line_id
                                    for move_lines in move_line_ids:
                                        if move_lines.account_id.id == account:
                                ...
                                tax_line_obj.create(cr, uid, vals, context=context)
        return True
...
class account_move_line(osv.Model):
    ...
    def reconcile(self, cr, uid, ids, type='auto', writeoff_acc_id=False, writeoff_period_id=False, writeoff_journal_id=False, context=None):
        ...
        res = super(account_move_line, self).reconcile(cr, uid, ids=ids, type = 'auto', writeoff_acc_id=writeoff_acc_id, 
            writeoff_period_id=writeoff_period_id, writeoff_journal_id = writeoff_journal_id, context=context)
